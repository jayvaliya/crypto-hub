...
const TaxCalculator = () => {
  ...
  const calculateTax = () => {
    ...
    if (transactionType === 'Buy' || transactionType === 'Sell') {
      taxRate = 0.3; // Flat 30% tax on crypto transactions
    } else if (transactionType === 'Mining' || transactionType === 'Staking') {
      taxRate = 0.3; // Flat 30% tax on crypto income (if sold/transferred)
    } else {
    ...
    if (holdingPeriodNum < 36) {
      taxRate = 0.05; // Flat 5% tax for short-term holdings
    }
    // Additional considerations
    const surchargeAndCess = 0.04; // Add 4% health and education cess
    taxRate += taxRate * surchargeAndCess;
